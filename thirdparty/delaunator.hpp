...
#ifndef ROOKIE_DELAUNATOR_HPP
#define ROOKIE_DELAUNATOR_HPP
...
// monotonically increases with real angle, but doesn't need expensive trigonometry
  ...
  inline std::size_t Delaunator::hash_key(const float x, const float y) const {
    ...
    return fast_mod(
      static_cast<std::size_t>(std::llround(std::floor(pseudo_angle(dx, dy) *
      static_cast<float>(m_hash_size)))),
      m_hash_size);
  }
